...
function HodINFT() {
  ...
  const fetchStocks = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/items/approve/inft");
      setStocks(res.data);
  ...
  const handleApproval = async (id, status) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/items/approve/inft/${encodedId}`, {
