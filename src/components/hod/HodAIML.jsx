...
function HodAIML() {
  ...
  // Fetch stocks for AIML department from the API
  const fetchStocks = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/items/approve/aiml");
      setStocks(res.data);
  ...
  const handleApproval = async (id, status) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/items/approve/aiml/${encodedId}`, {
    ...
    <div className="App">
      ...
      <div className="content">
        ...
        {selectedOptions === 'Main Page' && (
          <>
            <h1>AIML Approved Stock List</h1>
