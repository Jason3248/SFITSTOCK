...
import '../styles/hodelec.css';  // Make sure to include your CSS file
...
function HodELEC() {
  ...
  // Fetch ELEC department stocks
  const fetchStocks = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/items/approve/elec");
      setStocks(res.data);
  ...
  const handleApproval = async (id, status) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/items/approve/elec/${encodedId}`, {
        hodApprovalStatus: status,
        rejectionReason: reason
      });
      fetchStocks(); // Refresh the stocks after action
    } catch (err) {
    ...
    <div className="App">
      ...
      <div className="content">
        ...
        {selectedOptions === 'Main Page' && (
          <>
            <h1>ELEC Department Stock List</h1>
